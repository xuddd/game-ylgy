<template>
  <div class="home-index"
       ref="home">
    <div class="header">
      <h2>{{getTitle()}}</h2>
      <i class="el-icon-setting config"
         @click="isShowConfig=true"></i>
    </div>
    <div class="home-content">
      <BGRegion></BGRegion>

      <div class="content-wrapper">
        <template v-if="!isStarted">
          <div class="start-box">
            <div class="start-content">

            </div>
            <div class="start-bottom">
              <el-button type="primary"
                         size="medium"
                         @click="handleStart">开始游戏</el-button>
            </div>
          </div>
        </template>
        <template v-else>
          <div class="content"
               ref="cbr">
            <BlockRegion :theme="config.theme"
                         @clickBlock="handleClickBlock"
                         :height="blockHeight"
                         :width="blockWith"
                         ref="blockregion1"></BlockRegion>
          </div>
          <div class="content-bottom">
            <BlockRegion2 :theme="config.theme"
                          class="w-50"
                          @clickBlock="handleClickBlock"
                          :height="blockHeight"
                          :width="blockWith"
                          ref="blockregion2"></BlockRegion2>
            <BlockRegion3 :theme="config.theme"
                          class="w-50"
                          @clickBlock="handleClickBlock"
                          :height="blockHeight"
                          :width="blockWith"
                          ref="blockregion3"></BlockRegion3>
          </div>
          <div class="remove-blocks">
            <!-- <div class="rb" :style=""> -->
              <Block v-for="(block, index) in removedBlocks"
                   :key="index"
                   :option="block"
                   @clickBlock="handleClickRemovedBlock(block, index)"></Block>
            <!-- </div> -->
            
          </div>
          <div class="region">
            <Block v-for="(block,index) in bottomList"
                   :key="index"
                   :option="block"></Block>
          </div>
          <div class="footer">
            <span class="btn-block" :class="[removeDisabled ? 'disabled' : '']"
                  @click="handleRemove">
              <i class="btn1"></i>
            </span>
            <span class="btn-block" :class="[undoDisabled ? 'disabled' : '']"
                  @click="handleUndo">
              <i class="btn2"></i>
            </span>
            <span class="btn-block" :class="[resortDisabled ? 'disabled' : '']"
                  @click="handleResort">
              <i class="btn3"></i>
            </span>
          </div>
        </template>
      </div>
      <audio id="audio-bgm"
             v-show="false"
             controls
             loop
             volume="0.1"
             src="@/assets/audio/BGM.mp3">
      </audio>
      <audio id="audio-notice"
             v-show="false"
             src="@/assets/audio/咚.wav"
             :volume="1.0"></audio>
      <audio id="audio-notice2"
             v-show="false"
             :src="audioSrc"
             :volume="1.0"></audio>
    </div>
    <!-- 设置弹窗 -->
    <Dialog v-if="isShowConfig"
            @close="isShowConfig=false">
      <div class="config-content">
        <el-form :model="config"
                 label-width="50px">
          <el-form-item label="主题"
                        v-if="!isStarted">
            <el-select v-model="config.theme"
                       size="mini">
              <el-option label="海贼王"
                         value="hzw"></el-option>
              <el-option label="神奇宝贝"
                         value="sqbb"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="音乐">
            <el-switch v-model="config.bgm"
                       @change="handleBGMChange"></el-switch>
          </el-form-item>
          <el-form-item label="音效">
            <el-switch v-model="config.notice"
                       @change="handleNoticeCHange"></el-switch>
          </el-form-item>
        </el-form>
        <div class="config-footer">
          <el-button @click="handleBack"
                     type="danger">放弃挑战</el-button>
        </div>
      </div>
    </Dialog>
    <!-- 提示弹窗 -->
    <Dialog v-if="isShowDialog"
            @click="isShowDialog=false"
            :dialogConfig="{isShowClose: false}">
      <div class="finish-content">
        <p class="fc-title">{{isFinished ? "恭喜通关" : "闯关失败"}}</p>
        <div class="fc-content">
          <el-row>
            <el-button size="medium"
                       @click="handleBack">返回首页</el-button>
          </el-row>
          <el-row>
            <el-button size="medium"
                       @click="handleRestart">重新开始</el-button>
          </el-row>
        </div>
      </div>
    </Dialog>
  </div>
</template>

<script lang="ts">
import { Component, Vue } from "vue-property-decorator";
import Block from "./Block.vue";
import BlockRegion from "./BlockRegion.vue";
import BlockRegion2 from "./BlockRegion2.vue";
import BlockRegion3 from "./BlockRegion3.vue";
import BGRegion from "./BGRegion.vue";
import Dialog from "./Dialog.vue";
import { BlockModel } from "@/model/BlockModel";
@Component({
  components: {
    Block,
    BlockRegion,
    BlockRegion2,
    BlockRegion3,
    BGRegion,
    Dialog,
  },
})
export default class Home extends Vue {
  private audioSrc: string = "";
  private bottomList: any[] = [];
  private bgList: any = [];
  // 是否已经开始
  private isStarted = false;
  // 是否显示提示弹窗
  private isShowDialog = false;
  // 是否显示配置弹窗
  private isShowConfig = false;
  // 是否闯关成功
  private isFinished = false;
  private removeDisabled = false;
  private undoDisabled = false;
  private resortDisabled = false;
  private lastClickRegion: string = "";
  private lastClickBlock: BlockModel|null = null;
  // 配置对象
  private config = {
    bgm: true,
    notice: true,
    theme: "hzw",
  };
  // 移除的方块
  private removedBlocks: BlockModel[] = [];
  private audioResMap: Map<string, any> = new Map();
  private countMap: Map<string, number> = new Map();
  private blockWith = 0;
  private blockHeight = 0;
  private classSrc =
    "data:image/png;base64,";
  mounted() {
    this.audioResMap.set("remove", require("@/assets/audio/remove.wav"));
    this.audioResMap.set("finished", require("@/assets/audio/finished.wav"));
    this.audioResMap.set("unfinish", require("@/assets/audio/unfinish.wav"));
    this.countMap.set("blockregion0", 0);
    this.countMap.set("blockregion1", 0);
    this.countMap.set("blockregion2", 0);
    this.countMap.set("blockregion3", 0);
    this.getBGList();
  }
  handleStart() {
    this.bottomList = [];
    this.isStarted = true;
    this.$nextTick(() => {
      this.blockWith = ((this.$refs.cbr as any).offsetWidth * 0.9) / 7;
      this.blockHeight = ((this.$refs.cbr as any).offsetHeight * 0.95) / 8;
    });
    if (this.config.bgm) {
      this.handleMuted("audio-bgm", false);
    }
  }

  private handleMuted(id: string, isMuted: boolean, isPlay = true) {
    var audio: HTMLAudioElement = document.getElementById(
      id
    ) as HTMLAudioElement;
    audio.muted = isMuted;
    if (isPlay && !isMuted) {
      audio.play();
    }
  }

  private handleClickRemovedBlock(block: BlockModel, index: number) {
    this.removedBlocks.splice(index, 1);
    this.handleClickBlock(block, this.removedBlocks.length, "blockregion0");
  }

  private handleClickBlock(block: any, blockListCount: number, type: string) {
    this.lastClickRegion = type;
    this.lastClickBlock = block;
    this.countMap.set(type, blockListCount);
    this.noticeAudio();
    const oldBIndex = this.bottomList.findIndex((b) => b.type === block.type);
    // const lastIndex =  this.bottomList.lastIndexOf(block, oldBIndex);
    if (oldBIndex > -1) {
      const s = this.bottomList.slice(oldBIndex);
      this.bottomList.splice(oldBIndex);
      this.bottomList.push(block);
      this.bottomList.push(...s);

      if (
        s.length >= 2 &&
        s[0].type === block.type &&
        s[1].type === block.type
      ) {
        this.bottomList.splice(oldBIndex, 3);
        this.noticeAudio2("remove");
        let isFinish = true;
        this.countMap.forEach((v, k) => {
          if (v !== 0) {
            isFinish = false;
          }
        });
        if (isFinish) {
          this.noticeAudio2("finished");
          this.isFinished = true;
          this.isShowDialog = true;
        }
      }
    } else {
      this.bottomList.push(block);
    }
    if (this.bottomList.length >= 7) {
      this.noticeAudio2("unfinish");
      this.isFinished = false;
      this.isShowDialog = true;
    }
  }

  private handleBack() {
    this.isStarted = false;
    this.resetField();
  }
  private noticeAudio() {
    if (this.config.notice) {
      const audio: any = document.getElementById("audio-notice");
      audio.load();
      audio.play();
    }
  }

  private noticeAudio2(type: string) {
    const audio: any = document.getElementById("audio-notice2");
    audio.src = this.audioResMap.get(type);
    audio.play();
  }

  private getTop(row: number) {
    return `${(8 - row) * (40 + 35) - 40}px`;
  }

  private getLeft(col: number) {
    return `${col * (40 + 23)}px`;
  }

  private getBGList() {
    let list = [];
    for (let row = 0; row < 8; row++) {
      for (let col = 0; col < 6; col++) {
        let bgBlock = new BlockModel();
        bgBlock.rowIndex = row;
        bgBlock.colIndex = col;
        bgBlock.src = this.classSrc;
        list.push(bgBlock);
      }
    }
    this.bgList = list;
  }

  private handleNoticeCHange() {
    this.handleMuted("audio-notice", !this.config.notice, false);
  }

  private handleBGMChange() {
    this.handleMuted("audio-bgm", !this.config.bgm);
  }

  private handleRestart() {
    this.handleBack();
    this.$nextTick(() => {
      this.isStarted = true;
    });
  }

  private resetField() {
    this.isShowDialog = false;
    this.isShowConfig = false;
    this.isFinished = false;
    this.removeDisabled = false;
    this.undoDisabled = false;
    this.resortDisabled = false;
    this.lastClickRegion = "";
    this.lastClickBlock = null;
    this.removedBlocks = [];
    this.bottomList = [];
    this.countMap.forEach((v,k) => {
      this.countMap.set(k, 0);
    });
  }

  private getTitle() {
    let title = "";
    switch (this.config.theme) {
      case "hzw":
        title = "ONE PIECE";
        break;
      case "sqbb":
        title = "神奇宝贝";
        break;
      default:
        break;
    }
    return title;
  }

  // 重新排序功能
  private handleResort() {
    // this.$confirm("功能暂未开放，敬请期待！", "提示", {
    //   showCancelButton: false,
    // });
    if(!this.resortDisabled) {
      (this.$refs.blockregion1 as any).resort();
      (this.$refs.blockregion2 as any).resort();
      (this.$refs.blockregion3 as any).resort();
      this.resortDisabled = true;
    }
  }

  // 移除功能
  private handleRemove() {
    // this.$confirm("功能暂未开放，敬请期待！", "提示", { showCancelButton: false });
    if(!this.removeDisabled) {
      this.removedBlocks = this.bottomList.slice(0, 3);
      this.bottomList.splice(0, 3);
      this.removeDisabled = true;
    }
  }

  // 撤回功能
  private handleUndo() {
    // this.$confirm("功能暂未开放，敬请期待！", "提示", {
    //   showCancelButton: false,
    // });
    if(!this.undoDisabled) {
      (this.$refs[this.lastClickRegion] as any).undo();
      this.undoDisabled = true;
    }
  }
}
</script>
<style lang="scss" scoped>
.home-index {
  width: 100%;
  height: 100%;
  max-height: 844px;
  max-width: 390px;
  background: #ccff99;
  // background: url("~@/assets/images/home-bg.jpeg") 0 0 no-repeat;
  // background-size: 100% 100%;
  padding: 10px;
  overflow: hidden;
  position: relative;
  display: flex;
  flex-direction: column;
  justify-content: center;
  .header {
    position: relative;
    .config {
      position: absolute;
      right: 30px;
      top: 26px;
      font-size: 20px;
      cursor: pointer;
      &:hover {
        opacity: 0.7;
      }
    }
  }
  .home-content {
    flex: 1;
    display: flex;
    flex-direction: column;
    align-items: center;
    overflow: hidden;
    width: 100%;
    top: 0;
    left: 0;
    position: relative;
    .content-wrapper {
      position: absolute;
      top: 0;
      left: 0;
      height: 100%;
      width: 100%;
      display: flex;
      flex-direction: column;
      align-items: center;
      .start-box {
        display: flex;
        flex-direction: column;
        height: 100%;
        width: 100%;
        .start-content {
          height: 76%;
        }
        .start-bottom {
          height: 24%;
          text-align: center;
          display: flex;
          align-items: center;
          justify-content: center;
        }
      }
      .content {
        width: 350px;
        flex: 1;
        overflow: hidden;
        position: relative;
        display: flex;
        justify-content: center;
        ul {
          display: flex;
          flex-wrap: wrap;
          padding: 0;
        }
      }
      .content-bottom {
        height: 80px;
        width: 100%;
        display: flex;
        justify-content: space-between;
        .w-50 {
          width: 50%;
        }
      }
      .remove-blocks {
        height: 64px;
        width: 150px;
        display: flex;
        margin-bottom: 3px;
        .block {
          width: 33.3%;
          height: 100%;
        }
      }
      .region {
        width: 90%;
        min-width: 300px;
        max-width: 350px;
        display: flex;
        background: #965a1c;
        border: 10px #c1812f solid;
        height: 64px;
        align-items: center;
        padding: 0 1px;
        border-radius: 5px;
        .block {
          width: 13.9%;
          height: 97%;
        }
      }
      .footer {
        width: 350px;
        display: flex;
        justify-content: space-between;
        padding: 10px 30px;
        .btn-block {
          height: 60px;
          width: 60px;
          background: #6699ff;
          border-radius: 4px;
          display: flex;
          align-content: center;
          text-align: center;
          justify-content: center;
          align-items: center;
          cursor: pointer;
          &.disabled {
            background: #999999;
          }
          &:hover :not(.disabled) {
            opacity: 0.7;
          }
          i {
            width: 34px;
            height: 34px;
            display: block;
          }
          .btn1 {
            background: url("~@/assets/images/取出.png") 0 0 no-repeat;
            background-size: 100% 100%;
          }
          .btn2 {
            background: url("~@/assets/images/回退.png") 0 0 no-repeat;
            background-size: 100% 100%;
          }
          .btn3 {
            background: url("~@/assets/images/refresh.png") 0 0 no-repeat;
            background-size: 100% 100%;
          }
        }
      }
    }
  }
  .finish-content {
    display: flex;
    flex-direction: column;
    height: 100%;
    width: 100%;
    .fc-title {
      font-size: 44px;
      font-weight: bold;
    }
    .fc-content {
      flex: 1;
      margin-top: 15%;
      .el-row {
        margin: 20px 0;
      }
    }
  }
  .config-content {
    height: calc(100% - 70px);
    width: 100%;
    display: flex;
    flex-direction: column;
    justify-content: flex-start;
    align-items: center;
    padding: 50px 0 20px 0;
    .el-form {
      flex: 1;
      ::v-deep .el-form-item__content {
        text-align: right;
      }
    }
    .config-footer {
      height: 40%;
    }
  }
}
</style>
